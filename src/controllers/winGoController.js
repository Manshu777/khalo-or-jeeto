...
ORDER BY 
    ...
    if (results.length === 0) {
      return res.status(200).json({ message: 'No number bets found', data: [] });
    }
    
    return res.status(200).json(results);
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: 'Error fetching number betting stats' });
  }
};

const getMostNumberBets = async (req, res) => {
  try {
...
ORDER BY 
    ...
    console.error(error);
    return res.status(500).json({ message: 'Error fetching most number bets' });
  }
};
